...
def colour(pics, background):
    f = 2
    rgb = [0, 0, 0]
    for pic in pics:
        r, g, b = average_colour(pic[0])
        rgb[0] += r
        rgb[1] += g
        rgb[2] += b
    ...
    if grey > 450 and background:
        ...
        rgb_final = (
            int(rgb[1] / (len(pics) * f)),
            int(rgb[2] / (len(pics) * f)),
            int(rgb[0] / (len(pics) * f)),
        )
    else:
...
def list_pics(path, ignored):
    ...
    pics.sort()
    return pics, min_side, [100, 0.2, 0.2], area, equivalent, ignored


def print_collage(pics, background, dimensions, side, top):
    f = 3
    if background != 0:
        rgb = colour(pics, background)
    else:
        rgb = [0, 0, 0]
